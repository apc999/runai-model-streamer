...
from runai_model_streamer_alluxio.credentials.credentials import get_credentials, S3Credentials
import fnmatch
import os
import boto3
...
def glob(path: str, allow_pattern: Optional[List[str]] = None, credentials: Optional[S3Credentials] = None) -> List[str]:
    ...
    if not path.endswith("/"):
        path = f"{path}/"
    # Strip scheme prefix for list_files, then reattach the original scheme
    # (alluxio://) so downstream callers see consistent URIs.
    scheme_prefix = "alluxio://" if path.startswith("alluxio://") else "s3://"
    bucket_name, _, keys = list_files(s3,
                                       path=path,
                                       allow_pattern=allow_pattern)
    return [f"{scheme_prefix}{bucket_name}/{key}" for key in keys]
...
def list_files(
    ...
    # Accept both s3:// and alluxio:// (they share the backend plumbing here).
    path = removeprefix(path, 'alluxio://')
